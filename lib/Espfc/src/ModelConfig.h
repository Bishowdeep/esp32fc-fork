...
struct GpsConfig
{
  bool enabled = true;            // Enable GPS for your build
  uint8_t protocol = 0;           // 0 = NMEA (NEO-6M default)
  uint32_t baud = 9600;           // NEO-6M baud rate
  uint8_t port = 2;               // UART2 (Serial2)
  uint8_t minSats = 5;            // Min sats for valid fix (lower for testing)
